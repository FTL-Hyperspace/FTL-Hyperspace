...
float CrewMember_Extend::CalculateStat(CrewStat stat)
{
    ...
    if (it != statBoosts.end())
    {
        ...
        {
            if ((((statBoost.shipTarget == StatBoost::ShipTarget::PLAYER_SHIP && orig->currentShipId == 0) || (statBoost.shipTarget == StatBoost::ShipTarget::ENEMY_SHIP && orig->currentShipId == 1)) || (statBoost.shipTarget == StatBoost::ShipTarget::CURRENT_ALL && orig->currentShipId == statBoost.source->currentShipId) || (statBoost.shipTarget == StatBoost::ShipTarget::CURRENT_ROOM && orig->iRoomId == statBoost.source->iRoomId && orig->currentShipId == statBoost.source->currentShipId) || (statBoost.shipTarget == StatBoost::ShipTarget::ALL))
                && (statBoost.source != orig || statBoost.affectsSelf)
                && ((statBoost.source->iShipId == orig->iShipId && statBoost.crewTarget == StatBoost::CrewTarget::ALLIES) || (statBoost.source->iShipId != orig->iShipId && statBoost.crewTarget == StatBoost::CrewTarget::ENEMIES) || statBoost.crewTarget == StatBoost::CrewTarget::ALL)
    ...
    switch(stat)
    {
        ...
        case CrewStat::MOVE_SPEED_MULTIPLIER:
            ...
            {
                finalStat *= orig->GetShipObject()->GetAugmentationValue("SPEED_BOOST") - 1;
            }
