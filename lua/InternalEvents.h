...
struct InternalEvents
{
    ...
    static EventInfo GetEventInfo(Identifiers event)
    {
        static EventInfo eventInfo[Identifiers::UNKNOWN_MAX + 1] = 
        {
            ...
            {"function weapon_renderbox(ProjectileFactory& weapon, int cooldown, int maxCooldown, char* firstLine, char* secondLine, char* thirdLine) return Chain, firstLine, secondLine, thirdLine", 5, false},
            ...
            {"function tabbed_window_confirm(currentTab)", 1, false},
            {"function tabbed_window_undo(currentTab)", 1, false},
