...
| DAMAGE_AREA | `ShipManager ship`, `Projectile projectile`, `Pointf location`, `Damage damage`, `Evasion forceHit`, `bool shipFriendlyFire` | `Defines.Chain chain`, `Evasion forceHit`, `bool shipFriendlyFire` | Called when an area is about to be hit. Note: projectile is nil for beam weapons and boarding drones. |
| DAMAGE_AREA_HIT | `ShipManager ship`, `Projectile projectile`, `Pointf location`, `Damage damage`, `bool shipFriendlyFire` | `Defines.Chain chain` | Called when an area is hit. Note: projectile is nil for beam weapons and boarding drones. |
