...
| N/A | GET_AUGMENTATION_VALUE | `ShipManager ship`, `char augment`, `float value` | `Defines.Chain chain`, `float value` | Called when checking an augmentation's value |
...
| N/A | PROJECTILE_INITIALIZE | `Projectile projectile`, `WeaponBlueprint bp` | `None` | Called when a projectile is initialized |
| N/A | PROJECTILE_FIRE | `Projectile projectile`, `ProjectileFactory weapon` | `None` | Called when a projectile is fired |
...
| N/A | JUMP_ARRIVE | `ShipManager ship` | `None` | Called when a ship arrives at a beacon |
